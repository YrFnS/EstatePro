...
export function ListingModerationPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (selected) {
        setSelected(payload.listings.find((item) => item.id === selected.id) || selected);
      }
    } catch (error) {
                    ...
                    <div className="grid sm:grid-cols-[210px_minmax(0,1fr)]">
                      ...
                      <div className="p-5">
                        ...
                        <div className="mt-5 flex flex-wrap gap-2">
                          ...
                          {listing.listingStatus === "published" ? <Button variant="outline" size="sm" className="gap-1" onClick={() => openAction(listing, "archive")}><Archive className="h-3.5 w-3.5" />Archive</Button> : null}
                        </div>
                      </div>
                    </div>
                  </CardContent>
